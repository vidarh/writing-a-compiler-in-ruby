...
designed for incremental spec compliance. The project has 2434 commits and
is actively developed. It targets 32-bit x86 and
...
frontier is language spec compliance: 4/80 spec files fully passing (27%
individual test case pass rate: 269/982).
...
3. **rubyspec-language** (`make rubyspec-language`) — 80 spec files from
...
4. **spec/** (`make spec`) — 91 custom mspec-compatible test files in
   [spec/](../../spec/) (3092 total lines). Reduced test cases for
...
|--------|-------|
| Total commits | 2434 |
| Core compiler LoC | ~7,200 (compiler.rb + compile_*.rb + parser.rb + shunting.rb + emitter.rb) |
| Core library LoC | ~10,600 across 49 files in [lib/core/](../../lib/core/) |
| Rubyspec language files passing | 4/80 (5%) |
| Individual tests passing | 269/982 (27%) |
| Rubyspec language files "crashing" | 48/80 (see note below) |
| Rubyspec language files failing (not crashing) | 27/80 (34%) |
| Rubyspec language compile failures | 1/80 |
| Rubyspec regexp pass rate | 66/154 (42%) — 2/24 files fully passing |

**Note on "crash" classification**: "CRASH" in run_rubyspec means "no
summary line printed" — it does NOT necessarily mean segfault. Of 48
"crashing" language spec files, many actually ran some tests before
crashing. Many "crashes" are mid-test failures (missing methods,
unhandled control flow) rather than segfaults. Partial-crash files with
test output are nearly as actionable as the 27 FAIL files.
...
5. **Massive headroom**: From 4/80 passing (5%) to hypothetical 80/80 —
...
suite (80 files) is regularly tracked. The full rubyspec has 3,773 spec
...
Of the 48 "crashing" language specs, many produce partial output before
failing — these are nearly as actionable as the 27 FAIL files since they
produce diagnostic output showing where the failure occurs.

## Opportunities

- **Systematic crash triage**: The 48 crashing specs likely share common
...
- **run_rubyspec improvements**: The script (447 lines of bash with complex
...
- **Spec-by-spec fix workflow**: The 27 failing-but-not-crashing specs
...
- **Crash root-cause catalog**: Run all 48 "crashing" specs individually,
  capture partial output and crash signals, categorize by common causes.
  Many of these already produce partial output. This would produce
  a prioritized "fix X to unblock Y specs" map.

- **Failing spec priority matrix**: For the 27 failing specs, count
