...
- [x] `./run_rubyspec rubyspec/language/heredoc_spec.rb` shows the backslash test passing (14 of 16 pass, 2 remaining failures are eval/NameError-dependent)
  VERIFIED: Independently ran `./run_rubyspec rubyspec/language/heredoc_spec.rb` — 14 passed, 2 failed, 0 skipped (16 total). The 2 failures are eval/NameError-dependent as expected.
- [x] `make selftest` and `make selftest-c` both pass
  VERIFIED: Independently ran both. `make selftest`: Fails: 0. `make selftest-c`: Fails: 0.
- [ ] [docs/rubyspec_language.txt](../../rubyspec_language.txt) is updated via `make rubyspec-language` with no regression in overall pass count (272 or more individual tests passing)
  FAIL: `docs/rubyspec_language.txt` shows 269 passed, not 272+. The file IS updated (heredoc_spec shows P:14, confirming the heredoc fix landed), but the overall count is 269 due to hash_spec.rb dropping from 16→12 passes (rubyspec submodule update). The criterion as written requires 272+ and cannot pass at 269, regardless of root cause.
- [x] The heredoc body reader in tokens.rb does NOT contain its own escape handling or interpolation handling — all of that goes through `Quoted.expect_dquoted` (or an extension of it)
  VERIFIED: tokens.rb lines 1037-1058: interpolated heredocs go through `Quoted.expect_dquoted(@s, nil, term) { @parser.parse_defexp }` (line 1052). Pass 1 (lines 961-998) only collects raw characters and detects the marker — no escape handling or interpolation code present. Single-quoted heredocs correctly skip escape processing (`result = raw_body`).
- [x] No special-casing of escape handling by heredoc type — the escape branch is identical for regular and squiggly heredocs
  VERIFIED: The `squiggly` flag only affects marker detection (line 973 vs 984) and dedent (lines 1003-1035). Both squiggly and non-squiggly heredocs go through the identical `Quoted.expect_dquoted` call at line 1052. No `if squiggly` around any escape handling.
- [x] Dedent happens during tokenization BEFORE escape processing — escapes are resolved on already-dedented raw content
  VERIFIED: Code flow: (1) Pass 1 collects `raw_body` (lines 961-998), (2) dedent modifies `raw_body` at lines 1003-1035, (3) THEN `raw_body` is fed to `BufferedScanner` → `Quoted.expect_dquoted` for escape processing (lines 1037-1058). Dedent clearly precedes escape processing.
- [x] No use of `unget` or any scanner pushback mechanism for modified/constructed buffers — only unmodified tokens retrieved from the scanner may be ungotten
  VERIFIED: The dedented heredoc body is processed through `BufferedScanner` (line 1047), NOT through `unget`. The only `unget` in the heredoc section is `@s.unget(rest_of_line)` at line 1061, which pushes back unmodified characters read from the scanner (the remainder of the heredoc-opening line) — this follows the standard lookahead pattern used throughout tokens.rb (cf. `denom_str`, `num_str`, `ws_chars.reverse` at lines 271, 793, 839).
