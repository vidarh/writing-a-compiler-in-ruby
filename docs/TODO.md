...
- **Nested block variable capture bug** (`transform.rb:86`, `transform.rb:331`, `transform.rb:567`): Outer block **parameters** not correctly captured in nested blocks
  **Root cause**: Lambda/proc parameters are explicitly removed from environment (line 247) so nested lambdas cannot capture them
  **Workarounds**: Three `@bug` workarounds create aliases to force capture (all cause selftest-c crash if removed):
  ...
  **Test cases**: `spec/nested_blocks_capture.rb` (2 tests), `spec/simple_nested_capture.rb` (3 tests) - all fail
  **Example**: `[[1]].each do |arr| arr.each {|x| puts arr.length } end` - crashes, `arr` not captured
  **Investigation**: `docs/NESTED_BLOCK_CAPTURE_DEBUG.md`
  **Attempted fixes**: Adding parameters to scope causes infinite recursion; not removing from env causes compiler crash
